...
export default function TransactionsPage() {
  ...
  const [expStart, setExpStart] = useState(monthStartISO(now.getFullYear(), now.getMonth()))
  ...
  // ---- Export ----
  function computeRange(): { start: string; end: string; label: string; slug: string } {
    if (expScope === 'year') {
      return {
        start: `${expYear}-01-01`,
        end: `${expYear}-12-31`,
    ...
    if (expScope === 'week') {
      ...
      return { start: isoOf(mon), end: isoOf(sun), label: lbl, slug: `Minggu-${isoOf(mon)}` }
    }
    if (expScope === 'custom') {
      let a = expStart
      let b = expEnd
  ...
  async function handleExport(format: ExportFormat) {
    ...
    try {
      ...
      const openingForRange = balanceBefore(transactions, overrides, range.start)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Export modal */}
      <Modal open={exportOpen} title="Ekspor Laporan" onClose={() => setExportOpen(false)}>
        <div className="space-y-5">
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {([
                ...
                ['year', 'Tahunan'],
                ['custom', 'Custom'],
              ] as Array<[ExportScope, string]>).map(([val, lbl]) => (
                <button
                  key={val}
          ...
          {expScope === 'year' && (
            <div>
              <label className="mb-1.5 block text-sm font-medium text-fg">Tahun</label>
              <select
                value={expYear}
                onChange={(e) => setExpYear(Number(e.target.value))}
                className="w-full rounded-xl border border-hairline bg-canvas px-3 py-2.5 text-sm text-fg outline-none focus:border-accent-strong"
          ...
          {expScope === 'custom' && (
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-fg">Dari</label>
                <input
                  type="date"
                  value={expStart}
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-fg">Sampai</label>
                <input
                  type="date"
                  value={expEnd}
