...
export async function exportWord(
  ...
  const txTotal = new TableRow({
    children: [
      ...
      cell([txt('', { color: WHITE })], { fill: BLUE_DARK }),
      cell([txt('', { color: WHITE })], { fill: BLUE_DARK }),
      cell([txt(formatIDR(report.income), { bold: true, color: WHITE })], { fill: BLUE_DARK, align: R }),
    ...
    sections: [
      {
        children: [
          ...
          summaryLine('Saldo awal (sisa bulan lalu)', report.opening),
          summaryLine('Total pemasukan', report.income, POS),
          summaryLine('Total pengeluaran', report.expense, NEG),
          summaryLine('Selisih (net)', report.net),
